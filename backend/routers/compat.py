...
async def export_scan(scan_id: str, format: str = "json", current_user: dict = Depends(get_current_user)):
    ...
    if format.lower() == "csv":
        ...
        writer.writerow(["Engine", results.get("scan_engine", "")])
        for port in results.get("ports") or []:
            writer.writerow([port.get("port"), port.get("service"), port.get("state")])
...
async def get_cve_details(cve_id: str, current_user: dict = Depends(get_current_user)):
    ...
    cve_data = vulnerabilities[0].get("cve") or {}
...
async def run_bulk_scan_job(bulk_scan_id: str, user_id: str, targets: List[str], scan_type: str, options: Optional[dict] = None):
    from routers.scans import run_scan_engine
    options = dict(options or {})
    options.setdefault("preset", "fast")
    completed = 0
    failed = 0
    lock = asyncio.Lock()
    ...
    async def scan_target(target: str) -> dict:
        ...
        async with semaphore:
            ...
            try:
                ...
                result = {"target": target, "scan_id": scan_id, "status": status, "scan_engine": (scan_result or {}).get("scan_engine"), "vulnerabilities_count": len((scan_result or {}).get("vulnerabilities") or [])}
                async with lock:
    ...
    await db.bulk_scans.update_one({"id": bulk_scan_id}, {"$set": {"status": "completed", "completed": completed, "failed": failed, "results": results, "completed_at": datetime.now(timezone.utc).isoformat()}})
...
async def create_bulk_scan(scan_data: BulkScanCreate, background_tasks: BackgroundTasks, current_user: dict = Depends(get_current_user)):
    ...
    options = dict(scan_data.options or {})
    options.setdefault("preset", "fast")
...
async def list_bulk_scans(current_user: dict = Depends(get_current_user)):
    scans = await db.bulk_scans.find({"user_id": current_user["id"]}, {"_id": 0}).sort("created_at", -1).to_list(50)
...
async def create_scheduled_scan(payload: ScheduledScanCreate, current_user: dict = Depends(get_current_user)):
    ...
    await db.scheduled_scans.insert_one(doc)
    doc.pop("_id", None)
    return doc

@router.get("/scheduled-scans")
async def list_scheduled_scans(current_user: dict = Depends(get_current_user)):
    items = await db.scheduled_scans.find({"user_id": current_user["id"]}, {"_id": 0}).sort("created_at", -1).to_list(50)
    return {"scheduled_scans": items}
...
async def patch_scheduled_scan(schedule_id: str, enabled: Optional[bool] = None, current_user: dict = Depends(get_current_user)):
    ...
    if enabled is not None:
        await db.scheduled_scans.update_one({"id": schedule_id}, {"$set": {"enabled": enabled}})
...
async def run_scheduled_scan_now(schedule_id: str, background_tasks: BackgroundTasks, current_user: dict = Depends(get_current_user)):
    ...
    now = datetime.now(timezone.utc).isoformat()
    options = {"preset": "fast"}
    await db.bulk_scans.insert_one({"id": bulk_scan_id, "user_id": current_user["id"], "scheduled_scan_id": schedule_id, "scan_type": schedule["scan_type"], "targets": schedule["targets"], "options": options, "total_targets": len(schedule["targets"]), "completed": 0, "failed": 0, "status": "pending", "created_at": now})
